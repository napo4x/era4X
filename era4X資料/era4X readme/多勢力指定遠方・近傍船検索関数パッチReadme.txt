...
多勢力指定近傍船検索関数(近すぎ判定追加版)です。
同時に複数の勢力から選べる様になり・近すぎる船を除ける様にしたNEAREST_ANY_COUNTRY_SHIP_CHECKです。


@FURTHEST_MULTIPLE_ANY_COUNTRY_SHIP_CHECK
多勢力指定遠方船検索関数です。
近い船ではなく遠い船を探し・遠すぎる船を除ける様にしたNEAREST_MULTIPLE_ANY_COUNTRY_SHIP_CHECKです。
...
@NEAREST_MULTIPLE_ANY_COUNTRY_SHIP_SHOW
@FURTHEST_MULTIPLE_ANY_COUNTRY_SHIP_SHOW
NEAREST_MULTIPLE_ANY_COUNTRY_SHIP_CHECKのテスト用関数です。
指定した勢力の「自動移動で移動した時に閾値の範囲内で一番短い/遠い距離で移動するはずの位置の船」の国家IDとIDと名前と座標を表示するか、「その勢力に船は無い」旨を表示します。
一度に複数の勢力を指定する為にビット演算を利用している都合上、とりあえずはマイナスなどは選べなくなっていますが0および13～15番の勢力番号が選択可能です。


・SLG_SHOP.ERB
@SLG_SHOP
MOVING_DISTANCE_SHOWとNEAREST_ANY_COUNTRY_SHIP_SHOWとNEAREST_MULTIPLE_ANY_COUNTRY_SHIP_SHOWとFURTHEST_MULTIPLE_ANY_COUNTRY_SHIP_SHOWを呼び出せる様に手を加えてあります。
